...
func (g *GiteaPoster) ListComments(ctx context.Context) ([]Comment, error) {
	...
	for {
		...
		// Gitea's issue comments endpoint ignores page/limit on many versions and returns every
		// comment each time, so stop as soon as a page repeats one already seen.
