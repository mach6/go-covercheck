...
func TestCollectUncoveredLinesWithFiltering(t *testing.T) {
	...
	testFile := filepath.Join(os.TempDir(), "test_filtering.go")
...
/* comment */
	...
	tests := []struct {
	...
	}{
		{
			...
			fileName: testFile,
			// Should keep: 1(package), 3(import), 6(func), 7(fmt.Println), 9(if), 10(fmt.Println), 17(for), 19(fmt.Println)
			// Should filter: 2(empty), 5(comment), 8(empty), 11(}), 12(empty), 13-15(block comment), 16(empty), 18(comment), 20(})
