...
func GetFunctionCoverageForFile(filename string, coverageBlocks []cover.ProfileBlock) (int, int, error) {
	// Skip non-Go files
	if !strings.HasSuffix(filename, ".go") {
		return 0, 0, nil
	}

	functions, err := CountFunctionsInFile(filename)
	if err != nil {
	...
	functionsWithCoverage := MatchFunctionsWithCoverage(functions, coverageBlocks)

	total := len(functionsWithCoverage)
	covered := 0
